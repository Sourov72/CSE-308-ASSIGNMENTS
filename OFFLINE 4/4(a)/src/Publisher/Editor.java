...
public abstract class Editor {
    ...
    public void decreasePrice(String stockName, double value){
        stock = stockSearch(stockName);
        stock.setPrice(stock.getPrice() - value);
        changedvalue = "Stock " + stockName + "'s price has decrease by " + value;
    }
    ...
    public void change(String changeType, String count, String stockType){
        if(changeType.equals("I"))
            ...
            increasePrice(stockType, Integer.parseInt(count));
        notify(stockType, changedvalue);
    }

    public void show(){
        System.out.println("Available stocks: ");
        for(int i = 0; i < stockList.size(); i++){
            System.out.println(stockList.get(i).getName() + " " + stockList.get(i).getCount() + " " + stockList.get(i).getPrice());
        }
    }

    public abstract void notify(String stype, String s);
